...
public class TCPServer extends Thread {
	...
	public TCPServer(ServerBoard serverBoard,
			OnMessageReceived messageListener,
			DiffieHellmanModule diffieHellmanModule) {
	...
	public void run() {
		...
		try {
			...
			ServiceInfo pairService = ServiceInfo.create(SERVICE_TYPE,
					"ComputerSMS", port, 0, 0, values);
			...
			try {
				...
				String message = new String(Base64.getEncoder().encode(
						diffieHellmanModule.getPublicKey().getEncoded()));
				...
				while (!diffieHellmanModule.isConnected()) {
					...
					if (clientMessage != null) {
						...
						diffieHellmanModule.generateSecretKey(
								keyFactory.generatePublic(spec), true);
					}
					clientMessage = null;
				}
				...
				while (running) {
					...
					if (clientMessage != null && messageListener != null) {
						...
						// class
						if (diffieHellmanModule.isConnected()) {
							clientMessage = diffieHellmanModule
									.decryptString(clientMessage);
						}
